...
DeclareHoldAllComplete[CoreToolsHold, CoreToolsSequence];
MakeBoxes[CoreToolsHold[e_], fmt_] := MakeBoxes[e, fmt];

DeclareSequenceHold[CoreToolsSequence];
...
MakeBoxes[CodePane[args__], _] := CodePaneBoxes[args];

DeclareHoldAllComplete[CodePaneBoxes];

CodePaneBoxes[e_, opts___Rule] := CodePaneBoxes[e, $paneSize, opts];

CodePaneBoxes[e_, sz:Except[_Rule], opts___Rule] := Block[
  {$UseCoreBoxFormatting = False}, With[
  {charLimit = Ceiling @ Max[(0.95 * getTotalChars[sz]) - 3, 5]},
  {exprBoxes = ConstrainedMakeBoxes[Unevaluated @ e, charLimit, Max[charLimit - 5, 3], 8]},
  PaneBox[exprBoxes,
...
estimateCellSize[expr_] := Module[
  ...
  width = InputFormStringLength[$itemDisplayFn @ $itemFn @ expr, PrintPrecision -> $paneBoxPP] * $fontW;
...
MakeBoxes[CodeTooltip[a_, b_, sz_:Automatic], form_] :=
  CodeTooltipBoxes[MakeBoxes[a, form], MakeBoxes[CodePane[b, sz], form], Automatic];

MakeBoxes[CodeTooltip[a_, None, ___], form_] := MakeBoxes[a, form];
...
DeclareHoldAllComplete[niceGridBoxes];
...
DeclareHoldAll[processMaxWidthHeight];
